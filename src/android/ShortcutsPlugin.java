...
public class ShortcutsPlugin extends CordovaPlugin {
    ...
    public boolean execute(
        ...
        try {
            if (action.equals(ACTION_SUPPORTS_DYNAMIC)) {
                boolean supported = isApiLevelSupported();
                ...
                if (!isApiLevelSupported()) {
                    callbackContext.error("Unsupported Android API version");
                    return true;
                }
    ...
    private boolean isApiLevelSupported() {
    ...
    private ShortcutInfo buildDynamicShortcut(
        ...
        String iconResource = jsonShortcut.optString("iconResource");
        if (iconBitmap.length() > 0) {
            icon = Icon.createWithBitmap(decodeBase64Bitmap(iconBitmap));
        }
        if (iconResource.length() > 0) {
            int drawableId = 0;
            drawableId = cordova.getActivity().getResources().getIdentifier(iconResource, "drawable", cordova.getActivity().getClass().getPackage().getName());
            if (drawableId == 0) {
                drawableId = cordova.getActivity().getResources().getIdentifier(iconResource, "drawable", cordova.getActivity().getPackageName());
            }
            if (drawableId != 0) {
                icon = Icon.createWithResource(context, drawableId);
            }
        }
